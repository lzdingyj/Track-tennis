...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_csv', type=str, default='', help='path to ground-truth CSV for comparison')
    ...
    if args.output_video:
        if args.gt_csv:
            gt_label = pd.read_csv(args.gt_csv)
            write_pred_video(video_file, pred_dict, save_file=out_video_file, traj_len=args.traj_len, label_df=gt_label)
            write_pred_video_points(video_file, pred_dict, out_video_file.replace('.mp4', '_points.mp4'), label_df=gt_label)
        else:
            write_pred_video_points(video_file, pred_dict, out_video_file)
