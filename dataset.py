...
# data_dir = "/home/featurize/work/TrackNet_dataset_frame/"
data_dir = "/home/featurize/work/tennis_track_dataset/"


# data_dir = r'E:\tennis_track_dataset'
...
class Shuttlecock_Trajectory_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        assert data_mode in ['heatmap',
                             'coordinate'], f'Invalid data_mode: {data_mode}, should be heatmap or coordinate'
        assert bg_mode in ['', 'subtract', 'subtract_concat',
                           'concat'], f'Invalid bg_mode: {bg_mode}, should be "", subtract, subtract_concat or concat'
    ...
    def _get_rally_dict(self):
        ...
        rally_dict = {
            'i2p': {i: os.path.join(self.root_dir, self.split, rally_dir) for i, rally_dir in enumerate(rally_dirs)},
            'p2i': {os.path.join(self.root_dir, self.split, rally_dir): i for i, rally_dir in enumerate(rally_dirs)}
        }
        return rally_dict
    ...
    def _get_split(self, rally_dir):
        """ Extract the split name from a full rally directory path.
            e.g. '{root_dir}/test/game1/Clip1' -> 'test'
        """
        return os.path.relpath(rally_dir, self.root_dir).split(os.sep)[0]
    ...
    def _gen_rally_img_congif_file(self, file_name):
        ...
        for rally_i, rally_dir in tqdm(self.rally_dict['i2p'].items()):
            # ====================== 修复 1：0000.jpg ======================
            w, h = Image.open(os.path.join(rally_dir, f'0000.{IMG_FORMAT}')).size
    ...
    def _gen_input_from_rally_dir(self, rally_dir):
        ...
        csv_file = os.path.join(rally_dir, 'Label.csv')
        assert os.path.exists(csv_file), f'{csv_file} does not exist.'

        # ✅ 完全匹配你的 CSV：file name,visibility,x-coordinate,y-coordinate,status
        label_df = pd.read_csv(csv_file)

        # ✅ 读取正确列名 + 强制浮点 + 空值填 0
        x = label_df['x-coordinate'].astype(np.float32)
        y = label_df['y-coordinate'].astype(np.float32)
        v = label_df['visibility'].astype(np.float32)

        x = np.nan_to_num(x, nan=0.0)
        y = np.nan_to_num(y, nan=0.0)
        v = np.nan_to_num(v, nan=0.0)

        # ✅ 生成正确图片路径：0000.jpg、0001.jpg...
        frame_ids = np.arange(len(label_df))
        f_file = np.array([os.path.join(rally_dir, f'{f_id:04d}.{IMG_FORMAT}') for f_id in frame_ids])

        id = np.array([], dtype=np.int32).reshape(0, self.seq_len, 2)
        frame_file = np.array([], dtype=str).reshape(0, self.seq_len)
        coor = np.array([], dtype=np.float32).reshape(0, self.seq_len, 2)
        vis = np.array([], dtype=np.float32).reshape(0, self.seq_len)
        ...
        for i in range(0, len(f_file), self.sliding_step):
            ...
            for f in range(self.seq_len):
                if i + f < len(f_file):
                    idx = i + f
                    tmp_idx.append((rally_i, idx))
                    tmp_frames.append(f_file[idx])
                    tmp_coor.append([x[idx], y[idx]])
                    tmp_vis.append(v[idx])
                    last_idx = idx
                else:
                    if self.padding and last_idx >= 0:
    ...
    def __getitem__(self, idx):
        ...
        if self.frame_arr is not None:
            ...
            if self.frame_alpha > 0:
                ...
                if self.bg_mode:
                    median_img = np.median([cv2.imread(f) for f in frame_file], axis=0)[..., ::-1]
                ...
                if self.bg_mode == 'subtract':
                    ...
                    prev_img = np.array(prev_img.resize(size=(self.WIDTH, self.WIDTH)))
                    prev_img = np.moveaxis(prev_img, -1, 0)
            ...
            return data_idx.astype(np.int32), coor_pred, coor, vis_pred.reshape(-1, 1), vis.reshape(-1,
                                                                                                    1), inpaint.reshape(
                -1, 1)
        else:
...
class Video_IterableDataset(IterableDataset):
    ...
    def __process__(self, imgs):
        ...
        for i in range(self.seq_len):
            ...
            if self.bg_mode == 'subtract':
                img = Image.fromarray(np.sum(np.absolute(img - median_img), 2).astype('uint8'))
                img = np.array(img.resize(size=(self.WIDTH, self.WIDTH)))
                img = img.reshape(1, self.HEIGHT, self.WIDTH)
            elif self.bg_mode == 'subtract_concat':
                diff_img = Image.fromarray(np.sum(np.absolute(img - median_img), 2).astype('uint8'))
                diff_img = np.array(diff_img.resize(size=(self.WIDTH, self.WIDTH)))
                diff_img = diff_img.reshape(1, self.HEIGHT, self.WIDTH)
                img = np.array(img.resize(size=(self.WIDTH, self.WIDTH)))
                img = np.moveaxis(img, -1, 0)
                img = np.concatenate((img, diff_img), axis=0)
